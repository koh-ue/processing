/* coupled_pendulum's simulation
 * See the following website for how to derive equations
 *
...
float[] RK(float dt, float[] x) {
  //Runge-Kutta

  // dt * dx(t_n,x_n)
  float[] dx_k1 = sv_mul(dt, dx(x));

  // dt * (dx(t_(n+0.5),x_(n+0.5))
  float[] dx_k2 = sv_mul(dt, dx(vv_add(x, sv_mul(0.5, dx_k1))));

  // dt * (dx(t_(n+0.5),x_(n+0.5))
  float[] dx_k3 = sv_mul(dt, dx(vv_add(x, sv_mul(0.5, dx_k2))));

  // dt * (dx(t_(n+1),x_(n+1))
  float[] dx_k4 = sv_mul(dt, dx(vv_add(x, dx_k3)));
